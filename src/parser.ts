...
import { err, ok, Option, Result, resultAndThen, ResultKind, resultMap, toOption } from '@guardian/types';
...
const succeed = <A>(a: A): Parser<A> =>
    parser(_ => ok(a));

const fail = <A>(e: string): Parser<A> =>
    parser(_ => err(e));

const isObject = (a: unknown): a is Record<string, unknown> =>
	typeof a === 'object' && a !== null;

const run = <A>(pa: Parser<A>) => (a: unknown): Result<string, A> =>
    pa.run(a);

// ----- Basic Parsers ----- //

const parseString: Parser<string> = parser(a =>
    typeof a === 'string' ? ok(a) : err(`Unable to parse ${a} as a string`),
);

const parseNumber: Parser<number> = parser(a =>
    typeof a === 'number' && !isNaN(a) ? ok(a) : err(`Unable to parse ${a} as a number`)
);
...
const parseDate: Parser<Date> = parser(a => {
    if (typeof a === 'string' || typeof a === 'number' || a instanceof Date) {
        ...
        if (d.toString() === 'Invalid Date') {
            return err(`${a} isn't a valid Date`);
        }

        return ok(d);
    }

    return err(`Can't transform ${a} into a date`);
});

const maybe = <A>(pa: Parser<A>): Parser<Option<A>> => parser(a =>
    ok(toOption(pa.run(a))),
);

// ----- Data Structure Parsers ----- //

const parseField = <A>(field: string, pa: Parser<A>): Parser<A> => parser(a => {
    if (!isObject(a)) {
        return err(`Can't lookup field '${field}' on something that isn't an object`);
    }

    return pa.run(a[field]);
});

const parseIndex = <A>(index: number, pa: Parser<A>): Parser<A> => parser(a => {
    if (!Array.isArray(a)) {
        return err(`Can't lookup index ${index} on something that isn't an Array`);
    }
...
const parseAt = <A>(location: string[], pa: Parser<A>): Parser<A> => parser(a => {
    if (location.length === 1) {
        return parseField(location[0], pa).run(a);
    }

    if (location.length > 1) {
        return parseField(location[0], parseAt(location.slice(1), pa)).run(a);
    }

    return err(`I need a list of fields to lookup a location in object ${a}`);
});

const parseArray = <A>(pa: Parser<A>): Parser<A[]> => parser(a => {
    const f = (acc: A[], remainder: unknown[]): Result<string, A[]> => {
        ...
        return err(`Could not parse array item ${item}`);
    }
    ...
    return err(`Could not parse ${a} as an array`)
})
...
const andThen = <A, B>(f: (a: A) => Parser<B>) => (pa: Parser<A>): Parser<B> => parser(a =>
    resultAndThen<string, A, B>(x => f(x).run(a))(pa.run(a)),
);

const oneOf = <A>(pas: Parser<A>[]): Parser<A> => parser(a => {
    const f = (remPas: Parser<A>[], errs: string[]): Result<string, A> => {
        if (pas.length === 0) {
